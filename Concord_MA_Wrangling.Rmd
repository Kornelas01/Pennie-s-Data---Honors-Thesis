...
read csv file 
```{r}
Concord_data <- read.csv("Observations_of_flowering_phenology_in_Concord__Massachusetts__USA__1963-1993_.csv")
```
...
lets try to remove the X from the beginning of our year
```{r}
Concord_long$year <-  
  str_remove(
    Concord_long$year, "X")
...
attach tsn number - first create a test df that can be joined to concord long df. Test df will only include uniqe species name 
```{r}
...
Rename the spec.name column in one to one TSN to match test_df
```{r}
colnames(one_to_one_TSN)[colnames(one_to_one_TSN) == 'spec.name'] <- 'Scientific_name'

```

Change TSN from chr to numeric
Rejoin one to one TSN  and test_df 
```{r}
test_df <- test_df %>% 
...
one_to_one_TSN <-  one_to_one_TSN %>% relocate(TSN, .after = Scientific_name)
...
Create a TSN column in Concord_Long
```{r}
Concord_long <- Concord_long %>% 
  mutate(TSN = NA)
```
 
 For loop to match up TSN
```{r}
for (i in 1:nrow(Concord_long)){
  if (!is.na(Concord_long$TSN[i])){
Concord_long$TSN [i] == Concord_long$TSN [i]    
  }else {
    index <- which(test_df$Scientific_name == Concord_long$Scientific_name [i])
    Concord_long$TSN[i] <-  test_df$TSN[index]
  }
}
...
for (i in 1:nrow(Clean_Concord_Data)){
  if (is.na(Clean_Concord_Data$TSN[i])){
    index <- which(test_df$Scientific_name == Clean_Concord_Data$Scientific_name [i])
    Clean_Concord_Data$TSN[i] <-  test_df$TSN[index]
  }else {
    Clean_Concord_Data$TSN [i] == Clean_Concord_Data$TSN [i]
  }
}
```
